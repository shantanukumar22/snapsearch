...
# ── Config from env ───────────────────────────────────────────────────────────
DEFAULT_ROOT = os.path.expanduser(
    os.getenv("SCREENSHOTS_DIR", "~/Desktop/screenshots-demo")
)
...
# Unicode spaces Mac uses in screenshot filenames (narrow no-break space etc)
_UNICODE_SPACES = ["\u202f", "\u00a0", "\u2009", "\u2008", "\u2007"]


def sanitize_name(name: str) -> str:
    """Replace Mac unicode spaces with regular spaces in a filename."""
    for ch in _UNICODE_SPACES:
        name = name.replace(ch, " ")
    return name


def sanitize_folder(folder: Path) -> None:
    """
    Scan a folder and rename any files that contain unicode spaces.
    Called automatically on every scan so the agent never sees bad paths.
    """
    for p in folder.rglob("*"):
        if not p.is_file():
            continue
        clean = sanitize_name(p.name)
        if clean != p.name:
            dest = p.parent / clean
            if not dest.exists():
                p.rename(dest)
                logger.info("Auto-sanitized: %r → %r", p.name, clean)
...
def get_root(folder: str | None = None) -> Path:
    raw = folder or os.getenv("SCREENSHOTS_DIR", "~/Desktop/screenshots-demo")
...
async def call_tool(name: str, arguments: dict):
    ...
    # ── read_screenshot ───────────────────────────────────────────────────────
        ...
        arguments["path"] = sanitize_name(arguments["path"])
        p = Path(arguments["path"]).expanduser()
        if not p.exists():
            return err(f"File not found: {p}")
        if not is_image(p):
